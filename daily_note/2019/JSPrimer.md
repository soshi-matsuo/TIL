...
- for inはオブジェクトのプロパティを反復する構文だが、以下の理由で非推奨  
  - プロトタイプチェーンを遡及して、親オブジェクトのものまで含めて列挙可能なプロパティを反復する、という挙動なので、意図しない結果になりやすい
  - 代わりに`Object.keys()`でそのオブジェクト自身のプロパティを配列で取得し、`forEach()`で参照するのが無難
...
- `Object`の静的メソッド→オブジェクト型のインスタンスのメソッドとしてではなく、BIOの`Object`から直接呼び出せるメソッド
  ...
  - `Object.create()`:引数に渡したオブジェクトをプロトタイプオブジェクトに持つ、新規のオブジェクトを作成する  
    - SuperClassのプロトタイプオブジェクトを渡すことで、クラスの継承を表現することができる
